...
from .conftest import get_user_from_database, create_user_in_database
...
async def test_create_and_get_user(client: AsyncClient):
    ...
    response = await client.get(
        "/user/by-email",
        params={"email": "sheva@example.com"}
    )
...
async def test_update_user(client: AsyncClient, create_user_in_database, get_user_from_database):
    user_data = {
        "user_id": uuid4(),
        "name": "Vlad",
        "surname": "Zelenka",
...
async def test_update_only_one_user(client: AsyncClient, create_user_in_database, get_user_from_database):
    user_data_1 = {
        "user_id": uuid4(),
        "name": "Michael",
        "surname": "Jackson",
    ...
    user_1_from_db = dict(user_1_from_db[0])
    user_2_from_db = dict(user_2_from_db[0])

    assert user_1_from_db["name"] != user_2_from_db["name"]
    assert user_1_from_db["surname"] != user_2_from_db["surname"]
    assert user_1_from_db["email"] != user_2_from_db["email"]
    assert user_1_from_db["user_id"] != user_2_from_db["user_id"]

    assert user_1_from_db["is_active"] == user_2_from_db["is_active"]


async def test_delete_user(client: AsyncClient, create_user_in_database, get_user_from_database):
    user_data = {
        "user_id": uuid4(),
        "name": "Alex",
        "surname": "Shevchenko",
